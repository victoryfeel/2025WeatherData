  ...
  while (true) {
    ...
    for (int i = 0; i < infds; i++) {
      ...
      if (revent[i].data.fd == accept_socket) {
        // use while loop, accept() is blokcing function, so to set accept_socket nonblock.
        while (true) {
        ...
        printf("trigged write event\n");
        for (int a = 0; a < 10000000; ++a) {
          if (send(event.data.fd, "aaaaabbbbb55555", 15, 0) <= 0) {
            if (errno == EAGAIN) {
              printf("缓冲区已经填满.\n");
              break;
            }
